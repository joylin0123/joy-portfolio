  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      {crt && (
        <div
          aria-hidden
          className="fixed inset-0 pointer-events-none mix-blend-overlay opacity-30"
          style={{
            backgroundImage:
      ...
      <div className="mx-auto max-w-4xl px-5 py-8 md:py-12">
        ...
        <header className="mb-8 md:mb-12">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 md:w-20 md:h-20 grid place-items-center text-2xl font-black tracking-widest border">
                {initials}
              </div>
              <div>
            ...
            <div className="hidden md:flex flex-col items-end gap-2">
              <div className="flex items-center gap-2">
                <button onClick={() => setCrt((v) => !v)}>
                  <PixelBadge>CRT {crt ? 'ON' : 'OFF'}</PixelBadge>
                </button>
                <a
                  href="#"
                  onClick={(e) => {
              ...
              <p className="text-sm">
                Tip: toggle CRT or press<K>⌘</K>+<K>P</K> to print
              </p>
            </div>
          </div>
          ...
          <div className="mt-4 grid gap-2 text-sm md:text-base">
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
              <CustomLink href={`mailto:${resume.contacts.email}`}>
              ...
              <span className="text-slate-500">|</span>
              <CustomLink href={resume.contacts.linkedin}>
                linkedin.com/in/joy-lin-tw
              </CustomLink>
              <span className="text-slate-500">|</span>
              <CustomLink href={resume.contacts.github}>
                github.com/joylin0123
              </CustomLink>
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => copy(resume.contacts.email)}
                className="px-2 py-1 text-xs"
              >
                Copy email
              </button>
              <button
                onClick={() => copy(resume.contacts.github)}
                className="px-2 py-1 text-xs"
              >
                Copy GitHub
              </button>
        ...
        <Section title="Education">
          {resume.education.map((ed, idx) => (
            <article key={idx} className="bg-slate-900/40 p-4">
        ...
        <Section title="Projects">
          <div className="grid md:grid-cols-2 gap-4">
            {resume.projects.map((p, idx) => (
              <article key={idx} className="bg-slate-900/40 p-4 flex flex-col">
                <header className="mb-1 flex items-baseline justify-between gap-2">
                  ...
                  {p.link && (
                    <a
                      className="text-xs underline decoration-emerald-400/60 underline-offset-4 hover:text-emerald-200"
                      href={p.link}
                      target="_blank"
                      rel="noreferrer"
                    >
        ...
        <Section title="Skills">
          <div className="grid md:grid-cols-2 gap-4">
            {Object.entries(resume.skills).map(([group, items]) => (
              <div key={group} className="bg-slate-900/40 p-4">
